...
**Packages:** `packages/schema` · `packages/core` · `packages/skill` · `packages/cli` · **170 tests passing**
...
```bash
pnpm install
pnpm test      # 170 tests (builds first, then schema + core + skill + cli + examples/integrations)
pnpm build
pnpm release:smoke
