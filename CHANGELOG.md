...
- The drift check also pins the three `IG_TAG_SCANNER_*` assignment lines and the Node 20.13.0 remediation printf. The co-author test reads the workflow pattern from the executable line, skipping comments, and matches `cursoragent@cursor.com`, `noreply@anthropic.com`, and `[bot]`.
- Rendered markdown uses ASCII `--` where it used an em dash: drift-guard headings (`Drift guard -- hard block` and the confirm, warning, and trending forms), the session-brief heading, and the generated memory index. Consumers matching those strings need to update, and an on-disk INDEX.md reports stale once after upgrade until it is regenerated.
- The README version table now shows the no-leading-zero shape the action enforces. The old `^[0-9]+\.[0-9]+\.[0-9]+$` would accept `01.5.3`, which the 1.5.3 action release refuses.
- **Drift matching is now coverage-based, not any-shared-token.** Matches
  ...
  `partial_coverage` (default 0.3) live on `drift.thresholds`. Strong is
  coverage at or above `strong_coverage` with at least one non-noise
  evidence token. Path-category tokens count as ordinary evidence.
  Constraint matches still strip `CONSTRAINT_NOISE_TOKENS`. A slash-joined
  fragment in an out-of-scope item (`packages/cli`, `docs/`) is strong on
